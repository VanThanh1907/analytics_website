...
echo     [1/5] Recommendation API - Real Data (Port 5001)...
start "Recommendation-API" cmd /c "cd recommendation-engine && python api_server_real.py"
timeout /t 3 /nobreak > nul

echo     [2/5] HDFS NameNode UI (Port 9870)...
start "HDFS-UI" cmd /c "python hdfs_ui_simulation.py"
timeout /t 4 /nobreak > nul

echo     [3/5] Spark Master UI (Port 8080)...
start "Spark-UI" cmd /c "python spark_ui_fixed.py"
timeout /t 4 /nobreak > nul

echo     [4/5] E-commerce Web App (Port 5000)...
start "Web-App" cmd /c "cd web-app && python app.py"
timeout /t 5 /nobreak > nul

echo     [5/5] Analytics Dashboard (Port 8050)...
...
echo     🤖 Recommendation API:     http://localhost:5001 (100%% Real Data)
echo.
echo     ===============================================
...
echo        - Login: vanthanh (137 interactions)
echo        - Click vao san pham → Recommendations thay doi NGAY
echo        - 100%% du lieu that tu SQLite database
echo     5. http://localhost:5001 → Recommendation API (Real-time)
echo.
echo     💡 FIXED ISSUES:
echo     ✅ Dashboard hien thi ngay gio chinh xac
echo     ✅ E-commerce app co recommendation system
echo     ✅ Recommendation API voi 100%% du lieu that (SQLite)
echo     ✅ Instant category switch (30 phut)
echo     ✅ 1060+ real interactions trong database
